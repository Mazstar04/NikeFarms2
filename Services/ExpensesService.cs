...
namespace NikeFarms.v2._0.Services
{
    public class ExpensesService : IExpensesService
    {
        ...
        public ExpensesService(IExpensesRepository expensesRepository, IUserService userService, INotificationRepository notificationRepository, IUserRoleService userRoleService, IRoleService roleService, IFlockService flockService)
        {
        ...
        public Expenses Add(ExpensesDTO expensesDTO)
        {
            ...
            Notification notify = new Notification
            {
                ...
                Content = $"Manager {user.LastName} {user.FirstName} Spent: N{_flockService.Comma(expensesDTO.Price)} , On: {expensesDTO.Description}",
        ...
        public Expenses Update(ExpensesDTO expensesDTO)
        {
            ...
            if (notify != null)
            {
                var user = _userService.FindById(expensesDTO.UserId);
                var role = _roleService.FindByName("Admin");
                notify.Content = $"Manager {user.LastName} {user.FirstName} Spent: N{_flockService.Comma(expensesDTO.Price)} , On: {expensesDTO.Description}";
